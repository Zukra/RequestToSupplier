...
if ($arParams["SHOW_WORKFLOW"] || $this->startResultCache(false, [($arParams["CACHE_GROUPS"] === "N" ? false : $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $pagerParameters])) {
    if (! Loader::includeModule("iblock")) {
        ...
        if (
            ...
            if ($arParams["SET_BROWSER_TITLE"] === 'Y') {
                $browserTitle = \Bitrix\Main\Type\Collection::firstNotEmpty(
                    $arResult["PROPERTIES"], [$arParams["BROWSER_TITLE"], "VALUE"]
            ...
            if ($arParams["SET_META_DESCRIPTION"] === 'Y') {
                $metaDescription = \Bitrix\Main\Type\Collection::firstNotEmpty(
                    $arResult["PROPERTIES"], [$arParams["META_DESCRIPTION"], "VALUE"]
